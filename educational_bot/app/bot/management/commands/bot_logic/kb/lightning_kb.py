...
def building_main_menu_lightning(user_id):
    ...
    button = KeyboardButton(text="Молнии")
...
async def show_lightning_message_kb(callback_query, messages, message_index, user_id):
    ...
    if message.send_file and message.file_lightning:
        file_url = f"https://127.0.0.1:8000/media/{message.file_lightning}"
...
async def close_lightning_message(callback_query: types.CallbackQuery):
    ...
    if record_created:
        await callback_query.message.answer('Вы успешно ознакомились с молнией. Для ознакомления с остальными молниями нажмите кнопку Молнии')
    else:
        await callback_query.message.answer('Вы уже ознакомились с этой молнией.')
