...
from typing import (
    Union,
)  # add for cmd_start(message: Union[types.Message, types.CallbackQuery])
...
@sync_to_async
def log_user_action(user, action_type, action_details, action_text):
...
### сборка основного меню для пользователя
@sync_to_async
def building_main_menu(user_id):
    ...
    (
        ...
        .row(
            KeyboardButton(_("self-progress") + f" ({item_count})"),
            KeyboardButton(_("Settings")),
        )
    )
        ...
        match await user_exists(
            ...
            case 1:
                title, content, photo = await load_bot_logo(
                    "main_menu_logo", message.from_user.id
                )
                ...
                #Логирование действий пользователя
                await log_user_action(message.from_user, "main_menu", title, content)
            ...
            case 0:
                ...
                #Логирование действий пользователя
                await log_user_action(message.from_user, "welcome", title, content)

            case 2:
                title, content, photo = await load_bot_logo(
                    "user_blocked_logo", message.from_user.id
                )
                content = _(
                ...
                #Логирование действий пользователя
                await log_user_action(message.from_user, "user_blocked", title, content)
