...
async def handle_test_navigation(callback: CallbackQuery, callback_data: dict):
    try:
        ...
        # user_id = callback.from_user.id
        # start_image_path = await load_test_start_image(user_id)
        # media = types.InputFile(start_image_path) if start_image_path else None
...
#         sub_content = (
#             await counting_correct_answers(
#                 callback.from_user.id, callback_data.get("course_id")
#             )
#             if sub_content == "count_correct_answer"
#             else sub_content
#         )
