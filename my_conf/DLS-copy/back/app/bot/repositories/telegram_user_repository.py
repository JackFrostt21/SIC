...
class TelegramUserRepository(AsyncRepository[TelegramUser]):
    ...
    async def get_by_telegram_id(self, telegram_id: int) -> Optional[TelegramUser]:
        ...
        """
        return await self.get_by_filter(telegram_id=telegram_id)

    async def create_or_update_user(
        self, telegram_id: int, **user_data
    ) -> TelegramUser:
        """
    ...
    async def get_users_by_company(
        self, company_id: int, is_active: bool = True
    ) -> List[TelegramUser]:
        """
        ...
        if is_active:
            return await self.filter(
                company_id=company_id, state=TelegramUser.STATE_ACTIVE
            )
    ...
    async def get_or_create_with_company(
        self,
        telegram_id: int,
        username: str = None,
        full_name: str = None,
