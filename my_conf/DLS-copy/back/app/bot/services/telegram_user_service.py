...
class TelegramUserService:
    ...
    def __init__(
        self,
        user_repository: TelegramUserRepository = None,
        group_repository: TelegramGroupRepository = None,
    ):
        # Используем переданные репозитории или создаем новые (для обратной совместимости)
        self.user_repository = user_repository or TelegramUserRepository()
        self.group_repository = group_repository or TelegramGroupRepository()

    async def register_user(
        self, telegram_id: int, username: str = None, full_name: str = None, **user_data
    ) -> Dict[str, Any]:
        """
        ...
        """
        # Если передано полное имя, разбираем его
        if full_name:
            (
                user_data["last_name"],
                user_data["first_name"],
                user_data["middle_name"],
            ) = TelegramUser.parse_full_name(full_name)
    ...
    async def update_user_status(self, telegram_id: int, status: int) -> Dict[str, Any]:
        ...
        if status not in [
            TelegramUser.STATE_ACTIVE,
            TelegramUser.STATE_NOT_ACTIVE,
            TelegramUser.STATE_DELETED,
        ]:
    ...
    async def update_user_profile(
        self, telegram_id: int, **profile_data
    ) -> Dict[str, Any]:
        """
