...
class SettingsBotRepository(AsyncRepository[SettingsBot]):
    ...
    async def get_first_settings(self) -> Optional[SettingsBot]:
        ...
        settings_list = (
            await self.filter()
        )  # Может вернуть несколько, если нет фильтра # TODO: переделать

        settings_obj = await self.model_class.objects.afirst()
    ...
    async def get_test_result_image_path(
        ...
        image_field_name = "image_test_passed" if passed else "image_test_failed"
    ...
    async def get_admin_contact_message(self) -> Optional[str]:
        """
        Получает текст сообщения, которое показывается, если нет доступных курсов.
        Берёт сообщение из первых (или единственных) настроек.

        :return: Текст сообщения или None.
        """
        settings = await self.get_first_settings()
        if settings:
            return settings.admin_contact_message
