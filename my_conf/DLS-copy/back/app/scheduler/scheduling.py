...
from app.scheduler.models import ReminderSetting, SchedulerLog
from app.scheduler.services import send_unfinished_courses
...
def start_scheduler():
    """Запускает APScheduler и ставит периодическую задачу-поллер (каждую минуту)."""
...
def _get_settings():
    try:
        return ReminderSetting.objects.first()
    except Exception:
        logger.exception("Не удалось прочитать ReminderSetting")
        return None
...
def _compute_today_slots(settings_obj, now_dt):
    ...
    # ограничиваем 48 слотов в сутки
    ...
    try:
        ...
        logs = (
            SchedulerLog.objects.filter(
                task_name="send_unfinished_courses",
