...
class CourseContentService:
    ...
    async def get_open_courses(self, telegram_id: int) -> Dict[str, Any]:
        """
        Получает список открытых курсов, на которые пользователь еще не подписан.
        """
        ...
        # Получаем курсы из репозитория
        courses = await self.course_repository.get_open_courses_for_user(telegram_id)
    ...
    async def enroll_in_course(
        ...
        """
        Записывает пользователя на выбранный курс.
        """
        # Пробуем записать через репозиторий
        success = await self.course_repository.enroll_user_in_course(
        ...
        return {
            "success": False,
            "message": "Не удалось записаться на курс. Возможно, курс был удален или вы уже записаны.",
        }
    ...
    async def get_course_content(
        ...
        return {
            ...
            "course": {
                ...
                "image_path": (
                    course_data_from_repo["course"].image_course.path
                    if course_data_from_repo["course"].image_course
                    and hasattr(course_data_from_repo["course"].image_course, "path")
                    else None
                ),
