...
class TrainingCourse(BaseModel):
    ...
    obligatory = models.BooleanField(verbose_name="Обязательный", default=False)
    open_course = models.BooleanField(verbose_name="Открытый", default=False)
    notification_sent = models.BooleanField(verbose_name="Уведомление отправлено", default=False)
    ...
    certificate_validity_days = models.PositiveIntegerField(
        ...
        verbose_name="Срок действия сертификата, дней",
        help_text="Оставьте пустым, если сертификат по курсу не выдаётся.",
    )
...
class CourseTopic(BaseModel, OrderableMixin):
    ...
    description = models.TextField(null=True, blank=True, verbose_name="Описание")
    main_text = CKEditor5Field(
        config_name="default", null=True, blank=True, verbose_name="Основной текст"
    )
    ...
    main_text_readuser = models.BooleanField(
        default=False, verbose_name="Отображать основной текст в ТГ"
    )
    main_text_webapp_readuser = models.BooleanField(
        default=False, verbose_name="Отображать основной текст в ТГ в формате WebApp"
    )
    pdf_file_readuser = models.BooleanField(
        default=False, verbose_name="Отображать PDF файл в ТГ"
    )
    audio_file_readuser = models.BooleanField(
        default=False, verbose_name="Отображать аудио файл в ТГ"
    )
    video_file_readuser = models.BooleanField(
        default=False, verbose_name="Отображать видео файл в ТГ"
    )
    ...
    def has_main_text(self) -> bool:
        """
        Проверяет, есть ли в main_text реальный текст (а не пустые <p>&nbsp;</p>)
        """
        raw = self.main_text or ""
        clean = strip_tags(raw).replace("&nbsp;", "").strip()
        return bool(clean)

    def __str__(self):
        return f"{self.training_course.title} | {self.title}"
    
    # --- НОРМАЛИЗАЦИЯ ПУСТОГО HTML ---
    @staticmethod
    def _has_real_text(html: str | None) -> bool:
        """
        True, если в HTML есть содержательный текст (не <p>&nbsp;</p>, <br>, пустые теги).
        """
        raw = (html or "").strip()
        # убираем теги
        clean = strip_tags(raw)
        # убираем &nbsp; и неразрывный пробел \xa0
        clean = clean.replace("&nbsp;", "").replace("\xa0", "").strip()
        return bool(clean)

    def has_main_text(self) -> bool:
        """Удобный геттер для шаблонов/кода."""
        return self._has_real_text(self.main_text)

    def _normalize_main_text(self) -> None:
        """Если main_text пустышка — приводим к None"""
        if not self._has_real_text(self.main_text):
            self.main_text = None

    def clean(self):
        self._normalize_main_text()
        return super().clean()
    ...
    def more_info(self):
        return mark_safe(
            f"<a href='/admin/learning_app/coursetopic/{self.id}/change/'>"
            f" Перейти для редактирования: {self.id} </a>"
        )
...
class ScormPack(BaseModel):
    ...
    name_index = models.CharField(max_length=255, verbose_name="Стартовый файл SCORM", default="", blank=True)
    manifest_data = models.JSONField(
        blank=True, null=True, verbose_name="Манифест для веб"
    )
    ...
    def clean(self):
        super().clean()
        # TODO: Уточнить у заказчика, нужно ли проверять, что выбранная
        # тема (course_topic) относится к указанной программе обучения
        # (training_course). Если да — добавить здесь ValidationError.
    ...
    def extract_and_parse_manifest(self):
        ...
        if os.path.exists(manifest_path):
            try:
                ...
                super().save(update_fields=['manifest_data'])
            except Exception as e:
            ...
            ScormPack.objects.filter(pk=self.pk).update(
