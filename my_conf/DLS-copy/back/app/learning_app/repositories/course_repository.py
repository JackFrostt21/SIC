...
class CourseRepository(AsyncRepository[TrainingCourse]):
    ...
    async def get_open_courses_for_user(self, telegram_id: int) -> List[TrainingCourse]:
        """
        Получает список открытых курсов, на которые пользователь еще не записан.
        ...
        def _get_open_courses_sync():
            try:
                ...
                # Получаем курсы:
                # 1. open_course=True (открытый)
                # 2. is_actual=True (актуальный)
                # 3. Исключаем те, где пользователь уже есть (сам или в группе)
                courses = (
                    TrainingCourse.objects.filter(open_course=True, is_actual=True)
                    .exclude(Q(user=user) | Q(group__in=user_groups))
                    .distinct()
                    .order_by("title")
                )
                ...
                logging.getLogger(__name__).error(f"Error getting open courses: {e}")
                return []

        return await AsyncUnitOfWork.execute(_get_open_courses_sync)

    async def enroll_user_in_course(self, telegram_id: int, course_id: int) -> bool:
        """
        ...
        def _enroll_sync():
            try:
                ...
                # Добавляем пользователя в ManyToMany поле user курса
                course.user.add(user)
                return True
            except (TelegramUser.DoesNotExist, TrainingCourse.DoesNotExist) as e:
                logging.getLogger(__name__).error(f"Error enrolling user: {e}")
                return False
            except Exception as e:
                logging.getLogger(__name__).error(
                    f"Unexpected error enrolling user: {e}"
                )
                return False

        return await AsyncUnitOfWork.execute(_enroll_sync)
