...
class QuestionRepository(AsyncRepository[TopicQuestion]):
    ...
    async def get_questions_for_course(
        ...
        prefetch_answers: bool = True,
    ) -> List[TopicQuestion]:
        """
        Получает все актуальные вопросы для указанного курса или темы, отсортированные по порядку.
        course_id и topic_id взаимоисключительны.
        """

        def _get_questions_sync():
            if course_id and topic_id:
                raise ValueError("Нужно передать либо course_id, либо topic_id, но не оба.")
            if not course_id and not topic_id:
                return []

            qs = TopicQuestion.objects.filter(is_actual=True)
            if course_id:
                qs = qs.filter(training_id=course_id)
            if topic_id:
                qs = qs.filter(course_topic_id=topic_id)
